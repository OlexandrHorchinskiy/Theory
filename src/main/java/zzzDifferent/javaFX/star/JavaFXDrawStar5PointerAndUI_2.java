...
public class JavaFXDrawStar5PointerAndUI_2 extends Application {
// Глобальні змінні
    ...
    private int angleInclination = 0; // Кут нахилу зірки
...
// primaryStage це об'єкт вікна
    ...
    private void draw(Pane root) {
        ...
        rectangle.setFill(Color.rgb(250,250,250)); // фарбує прямокутник
...
// USER INTERFACE
        ...
        buttonDrawStar.setMinSize(160, 55);
        buttonDrawStar.setMaxSize(160, 55);
        ...
        Button buttonInclinationStar = new Button("Покрутити зірку за ГС"); // створює кнопку
        buttonInclinationStar.setTranslateX(410); // встановлює початкові координати
        buttonInclinationStar.setTranslateY(15); // встановлює початкові координати
        // призначає що буде виконуватись після натискання кнопки миші
        buttonInclinationStar.setOnMouseClicked(( event -> { // лямбда (lambda)
            angleInclination +=9;  // Змінює кут нахилу зірки

            draw(root);
        } ));

        Button buttonInclinationStar2 = new Button("Покрутити зірку проти ГС"); // створює кнопку
        buttonInclinationStar2.setTranslateX(410); // встановлює початкові координати
        buttonInclinationStar2.setTranslateY(45); // встановлює початкові координати
        // призначає що буде виконуватись після натискання кнопки миші
        buttonInclinationStar2.setOnMouseClicked(( event -> { // лямбда (lambda)
            angleInclination +=-9;  // Змінює кут нахилу зірки

            draw(root);
        } ));

        Button buttonDecreaseStar = new Button("Зменшити зірку"); // створює кнопку
        buttonDecreaseStar.setTranslateX(15); // встановлює початкові координати
        buttonDecreaseStar.setTranslateY(15); // встановлює початкові координати
        // призначає що буде виконуватись після натискання кнопки миші
        buttonDecreaseStar.setOnMouseClicked(( event -> { // лямбда (lambda)
            int decreaseOn = 15;
            starRadiusExternal += -decreaseOn; // Зменшує радіус зовнішнього кола
            starRadiusInternal += -decreaseOn * 0.37; // Зменшує радіус внутрішнього кола

            draw(root);
        } ));

        Button buttonIncreaseStar = new Button("Збільшити зірку"); // створює кнопку
        buttonIncreaseStar.setTranslateX(15); // встановлює початкові координати
        buttonIncreaseStar.setTranslateY(45); // встановлює початкові координати
        // призначає що буде виконуватись після натискання кнопки миші
        buttonIncreaseStar.setOnMouseClicked(( event -> { // лямбда (lambda)
            int increaseOn = 15;
            starRadiusExternal += increaseOn; // Збільшує радіус зовнішнього кола
            starRadiusInternal += increaseOn * 0.37; // Збільшує радіус внутрішнього кола

            draw(root);
        } ));


        int centralAxisX = 290; // Координати центру
        int centralAxisY = 300; // Координати центру
...
// Кути в градусах та радіанах між центральною вісю та вершинами
        int angleDegreeTop1 = angleInclination;
        double angleRadianTop1 = angleDegreeTop1 / 57.296;

        int angleDegreeTop2 = angleDegreeTop1 + angleBetweenBeams;
...
// Кути в градусах та радіанах між центральною вісю та основами
        int angleDegreeBottom1 = angleInclination + angleBetweenBeams / 2;
...
// Обчислення координат
        int rx = centralAxisX - centralAxisX;                                 // Координата точки відліку - координата точки центру
        int ryTop = ( centralAxisY - starRadiusExternal ) - centralAxisY;     // Координата точки відліку - координата точки центру
        int ryBottom = ( centralAxisY - starRadiusInternal ) - centralAxisY;  // Координата точки відліку - координата точки центру


// Координати вершин
        double top1X = ( centralAxisX + rx * Math.cos(angleRadianTop1) - ryTop * Math.sin(angleRadianTop1) );
        double top1Y = ( centralAxisY + rx * Math.sin(angleRadianTop1) + ryTop * Math.cos(angleRadianTop1) );

        double top2X = ( centralAxisX + rx * Math.cos(angleRadianTop2) - ryTop * Math.sin(angleRadianTop2) );
        double top2Y = ( centralAxisY + rx * Math.sin(angleRadianTop2) + ryTop * Math.cos(angleRadianTop2) );

        double top3X = ( centralAxisX + rx * Math.cos(angleRadianTop3) - ryTop * Math.sin(angleRadianTop3) );
        double top3Y = ( centralAxisY + rx * Math.sin(angleRadianTop3) + ryTop * Math.cos(angleRadianTop3) );

        double top4X = ( centralAxisX + rx * Math.cos(angleRadianTop4) - ryTop * Math.sin(angleRadianTop4) );
        double top4Y = ( centralAxisY + rx * Math.sin(angleRadianTop4) + ryTop * Math.cos(angleRadianTop4) );

        double top5X = ( centralAxisX + rx * Math.cos(angleRadianTop5) - ryTop * Math.sin(angleRadianTop5) );
        double top5Y = ( centralAxisY + rx * Math.sin(angleRadianTop5) + ryTop * Math.cos(angleRadianTop5) );


// Координати основ
        double bottom1X = ( centralAxisX + rx * Math.cos(angleRadianBottom1) - ryBottom * Math.sin(angleRadianBottom1) );
        double bottom1Y = ( centralAxisY + rx * Math.sin(angleRadianBottom1) + ryBottom * Math.cos(angleRadianBottom1) );

        double bottom2X = ( centralAxisX + rx * Math.cos(angleRadianBottom2) - ryBottom * Math.sin(angleRadianBottom2) );
        double bottom2Y = ( centralAxisY + rx * Math.sin(angleRadianBottom2) + ryBottom * Math.cos(angleRadianBottom2) );

        double bottom3X = ( centralAxisX + rx * Math.cos(angleRadianBottom3) - ryBottom * Math.sin(angleRadianBottom3) );
        double bottom3Y = ( centralAxisY + rx * Math.sin(angleRadianBottom3) + ryBottom * Math.cos(angleRadianBottom3) );

        double bottom4X = ( centralAxisX + rx * Math.cos(angleRadianBottom4) - ryBottom * Math.sin(angleRadianBottom4) );
        double bottom4Y = ( centralAxisY + rx * Math.sin(angleRadianBottom4) + ryBottom * Math.cos(angleRadianBottom4) );

        double bottom5X = ( centralAxisX + rx * Math.cos(angleRadianBottom5) - ryBottom * Math.sin(angleRadianBottom5) );
        double bottom5Y = ( centralAxisY + rx * Math.sin(angleRadianBottom5) + ryBottom * Math.cos(angleRadianBottom5) );
...
//
//        Button buttonPlayer = new Button("Player"); // створює кнопку
//        buttonPlayer.setTranslateX(300); // встановлює початкові координати
//        buttonPlayer.setTranslateY(10); // встановлює початкові координати
//        // призначає що буде виконуватись після натискання кнопки миші
//        buttonPlayer.setOnMouseClicked(( event -> { // лямбда (lambda)
//
...
// додає елементи в root
        root.getChildren().addAll(rectangle, text1, text2, text3, textField1, textField2, textField3,
                buttonDrawStar, buttonInclinationStar, buttonInclinationStar2, buttonDecreaseStar, buttonIncreaseStar,
